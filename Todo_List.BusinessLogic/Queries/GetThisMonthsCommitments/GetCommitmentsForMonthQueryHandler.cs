...
namespace Todo_List.BusinessLogic.Queries.GetThisMonthsCommitments
{
    public class GetCommitmentsForMonthQueryHandler : IRequestHandler<GetCommitmentsForMonthQuery, (IEnumerable<OneTimeCommitment>, IEnumerable<RecurringCommitment>)>
    {
        ...
        {
            var oneTimeCommitmentsForThisMonth = await _oneTimeCommitmentRepository.GetAllEntries().Where(otc => (otc.StartDate.Month == request.Month && otc.StartDate.Year == request.Year) || (otc.DueDate.HasValue && otc.DueDate.Value.Month == request.Month && otc.DueDate.Value.Year == request.Year)).ToListAsync();
            var recurringCommitmentsForThisMonth = await _recurringCommitmentRepository.GetAllEntries().Where(rc => (rc.StartDate.Month == request.Month && rc.StartDate.Year == request.Year) || (rc.DueDate.HasValue && rc.DueDate.Value.Month == request.Month && rc.DueDate.Value.Year == request.Year)).ToListAsync();
